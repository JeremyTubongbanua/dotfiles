...
- **Repo layout:** repos live in `~/GitHub/<name>/`. Most are standard single-root repos (`~/GitHub/<name>/.git`). A few are worktree collections — in those, `~/GitHub/<name>/trunk/` holds the upstream trunk checkout and sibling directories hold feature worktrees.
- **Worktree workflow** (for worktree-collection repos):
  ```sh
  cd ~/GitHub/<name>/trunk
  git fetch upstream
